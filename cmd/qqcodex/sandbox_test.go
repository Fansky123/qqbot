...
func TestValidateStartupUsesTrustedSandboxAndCapabilityProbe(t *testing.T) {
	...
	called := 0
	options.probe = func(_ context.Context, binary, codexBinary, workspace string) error {
		called++
		if binary != options.path || codexBinary != wantCodex || workspace != cfg.Consultation.Workspace {
			t.Fatalf("probe inputs = %q/%q/%q, want %q/%q/%q", binary, codexBinary, workspace, options.path, wantCodex, cfg.Consultation.Workspace)
		}
		return nil
	}
...
func TestValidateStartupRejectsUntrustedSandboxPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.mutate(t, &options)
			probeCalled := false
			options.probe = func(context.Context, string, string, string) error { probeCalled = true; return nil }
...
func TestValidateStartupRejectsSandboxProbeFailureAndTimeout(t *testing.T) {
	t.Run("failure", func(t *testing.T) {
		cfg, _ := validGitConfig(t)
		options := newTestSandboxOptions(t)
		options.probe = func(context.Context, string, string, string) error { return errors.New("private probe detail") }
	...
	t.Run("timeout", func(t *testing.T) {
		...
		options.probe = func(ctx context.Context, _, _, _ string) error { <-ctx.Done(); return ctx.Err() }
		started := time.Now()
		err := validateStartupWithSandbox(&cfg, options)
...
func TestConsultationSandboxProbeArgsUseProductionMountShape(t *testing.T) {
	args := codex.ConsultationSandboxProbeArgs("/host/bin/codex", "/host/workspace")
...
func TestInstalledBubblewrapCapabilityProbe(t *testing.T) {
	...
	if err := probeConsultationSandbox(ctx, productionSandboxPath, "/usr/bin/true", workspace); err != nil {
...
func newTestSandboxOptions(t *testing.T) sandboxStartupOptions {
	...
	return sandboxStartupOptions{
		...
		probe: func(context.Context, string, string, string) error { return nil },
	}
}
